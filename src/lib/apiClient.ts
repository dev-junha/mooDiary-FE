...
export const deleteDiary = async (diaryId: number): Promise<void> => {
  try { await api.delete(`/diaries/${diaryId}`); } 
...
export const createDiary = async (userId: number, content: string, imageFile?: File): Promise<DiaryDtoResponse> => {
  try {
    if (imageFile) {
      ...
      // Content-Type 헤더 제거 (Axios 자동 설정)
      const response = await api.post<DiaryDtoResponse>('/api/diaries/with-image', formData, {
        params: { userId }
      });
      return response.data;
    } else {
      ...
      const response = await api.post<DiaryDtoResponse>('/api/diaries', requestBody, {
        params: { userId }
      });
      return response.data;
    }
...
export const getUserDiaries = async (userId: number): Promise<DiaryDtoResponse[]> => {
  try {
    // /diaries/user/{userId} 혹은 /api/diaries/user/{userId} 확인 필요 (여기서는 /api/diaries 사용)
    const response = await api.get<any>(`/api/diaries/user/${userId}`);
    // 백엔드가 Page 객체(content 필드 등)를 반환할 수 있으므로 배열인지 확인 후 처리
