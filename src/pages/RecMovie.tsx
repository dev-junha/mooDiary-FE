...
export default function RecMovie() {
  ...
  // 페이지 로드 시 자동으로 영화 추천 가져오기 (한 번만 실행)
  useEffect(() => {
    if (
      emotionData &&
  ...
  const handleCategorySelect = async (category: string) => {
    ...
    // movie 카테고리인 경우 현재 페이지에서 API 호출
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        while (retryCount <= maxRetries && !success) {
          try {
            const recommendation = await createMovieRecommendation();
            if (recommendation) {
