...
import { getEmotionData, createBookRecommendation } from "@/lib/apiClient";
...
export default function RecBook() {
  ...
  // 페이지 로드 시 자동으로 책 추천 가져오기 (한 번만 실행)
  useEffect(() => {
    if (
      emotionData &&
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        while (retryCount <= maxRetries && !success) {
          try {
            const recommendation = await createBookRecommendation();
            if (recommendation) {
